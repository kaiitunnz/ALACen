...
def setup():
    if subprocess.call("conda install -y conda-forge::ffmpeg"):
    ...
    for d in dependencies:
        if subprocess.call(f"pip install {d}"):
    ...
    if subprocess.call("mfa model download dictionary english_us_arpa"):
        raise Exception(f"Failed to install MFA dictionary")
    if subprocess.call("mfa model download acoustic english_us_arpa"):
    ...
    if subprocess.call(f"pip install -e {diff2lip_dir / 'guided-diffusion'}"):
