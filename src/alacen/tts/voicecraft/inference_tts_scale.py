...
def inference_one_sample(
    ...
    gen_sample = audio_tokenizer.decode([(gen_frames, None)])
...
if __name__ == "__main__":
    ...
    for i, (audio_fn, text, prompt_end_frame, new_audio_fn, to_syn) in enumerate(
        tqdm.tqdm(
            (zip(audio_fns, texts, prompt_end_frames, new_audio_fns, text_to_syn))
        )
    ):
        ...
        seg_save_fn_gen = (
            f"{args.output_dir}/gen_{new_audio_fn[:-4]}_{i}_seed{args.seed}.wav"
        )
        seg_save_fn_concat = (
            f"{args.output_dir}/concat_{new_audio_fn[:-4]}_{i}_seed{args.seed}.wav"
        )
