...
def load_video_frames(path, args):
    ...
    command = (
        "ffmpeg -loglevel error -y -i {} -vf fps={} -q:v 2 -qmin 1 {}/%05d.jpg".format(
            in_path, args.video_fps, out_dir
        )
    )
...
def my_voxceleb2_crop(img):
    return img[
        : -int(img.shape[0] * 2.36 / 8),
        int(img.shape[1] * 1.8 / 8) : -int(img.shape[1] * 1.8 / 8),
    ]
...
def my_voxceleb2_crop_bboxs(img):
    return (
        0,
        img.shape[0] - int(img.shape[0] * 2.36 / 8),
        int(img.shape[1] * 1.8 / 8),
        img.shape[1] - int(img.shape[1] * 1.8 / 8),
    )
    ...
    if resize:
        if args.is_voxceleb2:
            results = [
                [
                    cv2.resize(
                        my_voxceleb2_crop(image), (args.image_size, args.image_size)
                    ),
                    my_voxceleb2_crop_bboxs(image),
                    True,
                ]
            ...
            results = [
                [
                    cv2.resize(image[y1:y2, x1:x2], (args.image_size, args.image_size)),
                    (y1, y2, x1, x2),
                    True,
                ]
                for image, (x1, y1, x2, y2) in zip(images, boxes)
...
def generate(
    ...
    face_frames = torch.FloatTensor(
        np.transpose(
            np.asarray(
                [face_det_results[i][0] for i in range(min_frames)], dtype=np.float32
            )
            / 255.0,
            (0, 3, 1, 2),
        )
    )  # [N, C, H, W]
    wrong_all_indiv_mels = torch.FloatTensor(
        np.asarray(wrong_all_indiv_mels[:min_frames])
    ).unsqueeze(
        1
    )  # [N, 1, h, w]
    ...
    if save_orig:
        if out_path is None:
            out_path_orig = os.path.join(
                args.sample_path,
                splitext(basename(video_path))[0]
                + "_"
                + splitext(basename(audio_path))[0]
                + "_orig.mp4",
            )
        else:
        ...
        torchvision.io.write_video(
            out_path_orig,
            video_array=torch.from_numpy(np.array(video_frames)),
            fps=args.video_fps,
            video_codec="libx264",
            audio_array=torch.from_numpy(wrong_audio_wavform).unsqueeze(0),
            audio_fps=args.sample_rate,
            audio_codec="aac",
        )
    ...
    if args.sampling_ref_type == "gt":
        ...
        ref_frames = face_frames[rand_idx : rand_idx + 1].repeat(
            len(face_frames), 1, 1, 1
        )
    ...
    for i in range(start_idx, end_idx, b_s * args.nframes):
        ...
        if (slice_end - i) % args.nframes == 0:
            ...
            wrong_indiv_mel_batch = wrong_indiv_mel_batch.reshape(
                -1,
                args.nframes,
                wrong_indiv_mel_batch.size(-3),
                wrong_indiv_mel_batch.size(-2),
                wrong_indiv_mel_batch.size(-1),
            )
        # except:
            ...
            img_batch = face_frames[i:slice_end]  # [BF, C, H, W]
            img_batch = img_batch.reshape(
                -1, 1, img_batch.size(-3), img_batch.size(-2), img_batch.size(-1)
            )
            ref_batch = ref_frames[i:slice_end]
            ref_batch = ref_batch.reshape(
                -1, 1, ref_batch.size(-3), ref_batch.size(-2), ref_batch.size(-1)
            )
            wrong_indiv_mel_batch = wrong_all_indiv_mels[i:slice_end]  # [BF, 1, h, w]
            wrong_indiv_mel_batch = wrong_indiv_mel_batch.reshape(
                -1,
        ...
        recon_batch = sample * mask + (1.0 - mask) * img_batch  # [BF, C, H, W]
        recon_batch = (
            (normalise(recon_batch) * 255).cpu().numpy().transpose(0, 2, 3, 1)
        )  # [-1,1] -> [0,255]
    ...
    dist.all_gather_object(
        all_generated_video_frames, generated_video_frames
    )  # gather not supported with NCCL
    all_generated_video_frames_combined = []
    [
        all_generated_video_frames_combined.extend(gvf)
        for gvf in all_generated_video_frames
    ]
    ...
    if dist.get_rank() == 0:
        print(
            ...
            t2 - t1,
            ",time without all  gather, ",
            t3 - t1,
            ",frames/gpu, ",
            len(generated_video_frames),
            ",total frames, ",
            min_frames,
        )
        print(wrong_audio_wavform.shape, np.array(generated_video_frames).shape)
        min_time = (
            len(generated_video_frames) / args.video_fps
        )  # because video is already smaller because it got chopped accoding to the mel array length
        ...
        torchvision.io.write_video(
            out_path,
            video_array=torch.from_numpy(np.array(generated_video_frames)),
            fps=args.video_fps,
            video_codec="libx264",
            audio_array=torch.from_numpy(wrong_audio_wavform).unsqueeze(0),
            audio_fps=args.sample_rate,
...
def main():
    ...
    if args.generate_from_filelist:
        generate_from_filelist(
            args.test_video_dir, args.filelist, model, diffusion, detector, args
        )
    else:
        generate(
            args.video_path,
